...
import { ArrowLeft, Phone, Mail, MapPin, MessageCircle, Send, Clock, User, GitBranch, Edit3, Save, X, Plus, StickyNote, Tag as TagIcon } from 'lucide-react'
...
export default function LeadDetail() {
  ...
  useEffect(() => {
    if (!id || !accountId) return
    setLoading(true)
    Promise.all([loadLead(), fetchFunnels(accountId).then(setFunnels), fetchUsers(accountId).then(setUsers), fetchTags(accountId).then(setTags)])
      .finally(() => setLoading(false))
  }, [id, accountId, loadLead])
      ...
      <div className="lead-detail">
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
            {(['chat', 'notes', 'history'] as const).map(tab => (
              <button key={tab} className={`btn btn-sm ${activeTab === tab ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setActiveTab(tab)}>
                {tab === 'chat' ? <><MessageCircle size={12} /> Chat ({messages.length})</> : tab === 'notes' ? <><StickyNote size={12} /> Notas ({notes.length})</> : <><GitBranch size={12} /> Historico</>}
              </button>
            ))}
          </div>
