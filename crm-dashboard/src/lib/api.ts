...
export async function apiFetch<T = any>(path: string, opts: RequestInit = {}): Promise<T> {
  ...
  if (res.status === 401) { localStorage.removeItem('dros_crm_token'); window.location.href = '/login'; throw new Error('Unauthorized') }
...
export interface Lead {
  ...
  wa_remote_jid: string | null; is_active: number; created_at: string; updated_at: string
  stage_name?: string; stage_color?: string; attendant_name?: string
...
export interface WhatsAppInstance { id: number; account_id: number; instance_name: string; api_url: string; api_key: string; status: string; phone_number: string | null }
...
export const createWhatsAppInstance = (accountId: number, data: { instance_name: string; api_url: string; api_key: string }) => apiFetch(`/api/integrations/whatsapp?account_id=${accountId}`, { method: 'POST', body: JSON.stringify(data) })
