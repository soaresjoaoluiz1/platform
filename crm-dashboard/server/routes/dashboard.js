...
router.get('/agents', requireRole('super_admin', 'gerente'), (req, res) => {
  ...
  const agents = db.prepare(`
    SELECT u.id, u.name, u.is_active,
      ...
      (SELECT COUNT(*) FROM leads l JOIN funnel_stages fs ON l.stage_id = fs.id WHERE l.attendant_id = u.id AND fs.is_conversion = 1 AND l.created_at >= ?) as conversions
    FROM users u WHERE u.account_id = ? AND u.role = 'atendente'
    ORDER BY leads_period DESC
  `).all(sinceStr, sinceStr, req.accountId)
